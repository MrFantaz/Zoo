...
public class Main implements IAnimalDeadListener {
	...
	public void jumpAll() {
		...
		for (int i = 0; i < predatocCage.getCage().size(); i++) {
			if (maxJump < predatocCage.getCage().get(i).jump()) {
				maxJump = predatocCage.getCage().get(i).jump();
				maxElement = i;
			}
		}
		if (predatocCage.getCage().size() == 1) {
			System.out.println("Один в поле воин!?");
		} else {
			System.out.println("Максимальная длина= " + maxJump + " " + predatocCage.getCage().get(maxElement).toString());
		}
	}
	...
	public void feedAnimal() {
		List<Predatoc> ani=predatocCage.getCage();
		if (ani.size() == 0) {
